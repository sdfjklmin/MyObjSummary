...
###### 1. 分析慢日志
    ...
    cli>
        ...
        ->结果
        1) 1) (integer) 1           #慢日志ID
           2) (integer) 1600929891  #执行时间
           3) (integer) 13          #执行耗时,以微秒为单位
           4) 1) "get"              #执行的命令和参数
              2) "big_data"
        2) 1) (integer) 0
           2) (integer) 1600929883
           3) (integer) 11
           4) 1) "config"
              2) "set"
...
###### 2. 存储较大的KEY
    ...
    #使用命令 --bigkeys, -i 代表时间间隔,单位秒(线上扫描会导致QPS突增) 
    ./redis-cli -h 127.0.0.1 -p 6379 --bigkeys
###### 3. KEY集中过期
    平时在使用Redis时没有延时比较大的情况，但在某个时间点突然出现一波延时，
    而且报慢的时间点很有规律，例如某个整点，或者间隔多久就会发生一次
    如果有大量的key在某个固定时间点集中过期，在这个时间点访问Redis时，就有可能导致延迟增加
    将 过期时间 添加 随机数
    set key value expire+random(100)
###### 4. 内存上限
    内存达到 maxmemory 的配置后,
    会先执行 淘汰策略(见 缓存淘汰策略),再新增数据
###### 5. RDB和AOF重写功能
    规划好数据备份的周期，建议在从节点上执行备份，而且最好放在低峰期执行。
    如果对于丢失数据不敏感的业务，那么不建议开启AOF和AOF重写功能
###### 6. 频繁使用 Swap
    调整内存空间,保证 redis 有足够的运行内存
###### 7. 网卡负载过高
    凉拌
###### 8. 总结
    redis 自身机制出发考虑问题
    系统出发,如: CPU、内存、网络，磁盘 
    提前预警: 对必要的指标进行提前预警,如:内存爆满
